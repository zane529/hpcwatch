...
class CollectNode(object):
    ...
    def collect_node_mem_info(self):
        ...
        mem_use = Gauge('node_mem_used', 'The mem ues of node', ['instance', 'instance_type'], registry=registry)
        mem_use.labels(instance=instance_id, instance_type=instance_type).set(mem_info.get('mem_used'))
        
        mem_per_used = Gauge('node_mem_used_per', 'The mem use percent of node', ['instance', 'instance_type'], registry=registry)
    ...
    def collect_node_disk_info(self):
        ...
        disk_used = Gauge('node_disk_used', 'The disk use of node', ['instance', 'instance_type'], registry=registry)
        disk_used.labels(instance=instance_id, instance_type=instance_type).set(disk_info.get('disk_used'))
        
        disk_used_per = Gauge('node_disk_used_per', 'The disk use percent of node', ['instance', 'instance_type'], registry=registry)
